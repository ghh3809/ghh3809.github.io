...
根据以上原理，我们可以使用代码构建next表。与手动计算方法不同的是，可以通过使用已构建的next表加速对比计算。
...
与next表的构建相似，KMP算法也是通过相似的方法进行匹配计算：
...
由于文本串指针永远不会后退，模式串指针只可能进行加1操作或者next操作（相等于做减法），因此该方法最多只可能进行2*n次操作，因此KMP算法中除next表构建外，其时间复杂度为O(n)。同理可以得到，next表的构建最多需要O(m)时间，因此KMP算法的时间复杂度不超过O(m+n)。同时对于构建好的next表，只要模式串不发生变化，就可以连续不断使用，这也使得渐进复杂度接近于O(n)。